...
struct nodeDE //Estrutura nó, que representa cada elemento da lista.
{
	
	int valor; //Valor armazenado na lista.
	struct nodeDE *prox, *prev; //Ponteiros para o próximo nó e o nó anterior.
	
};typedef struct nodeDE nodeDE;

class ListDE
{
	private:
		
		nodeDE *raiz; //Nó inicial da lista.
		nodeDE *last; //Nó final da lista.
	...
	public:
		...
		void printList()
		{
			...
			nodeDE *aux = raiz; //Nó auxiliar para percorrer a lista.
		...
		void printListReverse()
		{
			...
			nodeDE *aux = last; //Nó auxiliar para percorrer a lista.
		...
		{
			nodeDE *novo = (nodeDE *)malloc(sizeof(nodeDE)); //Aloca memória para o novo nó.
			novo->valor = v;
			novo->prox = NULL;
			novo->prev = NULL;
		...
		void removeValue(int v)
		{
			...
			while(aux != NULL)
			{
				if(aux->valor == v)
				{
					nodeDE *aux2 = aux->prev; //Nó auxiliar armazena o nó anterior ao atual.
		...
		void removeIndex(int n)
		{
		    ...
		    {
			   nodeDE *aux2 = aux->prox; //Nó auxiliar para poder atualizar os nós.
